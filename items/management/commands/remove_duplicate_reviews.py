...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        review_ids_to_delete = list(reviews.exclude(id=F('duplicate_id')).values_list('pk', flat=True))

        # Delete duplicate reviews
        Review.objects.filter(id__in=review_ids_to_delete).delete()

        self.stdout.write(self.style.SUCCESS(f'Successfully deleted {len(review_ids_to_delete)} duplicate reviews'))
