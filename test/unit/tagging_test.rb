...
class TaggingTest < Test::Unit::TestCase
  ...
  def test_find_tagged_with
    ...
    result1 = [@obj1]
    assert_equal Item.tagged_with("ipa"), result1
    assert_equal Item.tagged_with("ipa lager"), result1
    assert_equal Item.tagged_with("ipa", "lager"), result1
    
    result2 = [@obj1.id, @obj2.id].sort
    assert_equal Item.tagged_with("seasonal").map(&:id).sort, result2
    assert_equal Item.tagged_with("seasonal lager").map(&:id).sort, result2
    assert_equal Item.tagged_with("seasonal", "lager").map(&:id).sort, result2
  end
